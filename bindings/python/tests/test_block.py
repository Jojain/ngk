    ...
    for index, edge in enumerate(edge_occurrences):
        for other in edge_occurrences[index + 1 :]:
            if edge == other:
                ...
                edge_points = {edge.start.point.as_tuple(), edge.end.point.as_tuple()}
                other_points = {other.start.point.as_tuple(), other.end.point.as_tuple()}
                assert edge_points == other_points
                break
        if found_equal_pair:
            break
