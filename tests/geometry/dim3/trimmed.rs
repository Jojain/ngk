...
use ngk::geometry::{
    Curve, Interval, LINEAR_TOLERANCE, Plane, Point3, PointCoincidence, TrimmedCurve,
};
...
fn a_span_traverses_its_own_extent_from_fraction_zero_to_one() {
    let quarter = TrimmedCurve::new(unit_circle(), Interval::new(0.0, FRAC_PI_2));
...
fn two_arcs_between_the_same_points_are_told_apart_only_by_their_spans() {
    ...
    let minor = TrimmedCurve::new(unit_circle(), Interval::new(0.0, FRAC_PI_2));
    let major = TrimmedCurve::new(unit_circle(), Interval::new(0.0, FRAC_PI_2 - TAU));
...
fn reversing_a_span_keeps_its_geometry_and_swaps_its_ends() {
    let arc = TrimmedCurve::new(unit_circle(), Interval::new(0.0, FRAC_PI_2));
