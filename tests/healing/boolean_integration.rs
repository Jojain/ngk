...
fn healing_a_tangent_union_fuses_the_fragments_the_imprint_created() {
    ...
    assert_eq!(raw, (8, 13, 7), "splitting leaves eight fragments");
    ...
    // rather than eleven. The far cap's rim is a whole circle that nothing meets
    // along the way, so the place it closes stays classified inside that edge
    // and is not one of the six corners either.
    //
    // Euler still closes, over the cells the map holds rather than the entities
    // counted here: put that closure point back and the sum is
    // 7 - 10 + (5 disks + 1 annulus) = 7 - 10 + 5 = 2, exactly what the seamed
    // reading got as 7 - 11 + 6.
    assert_eq!(
        healed,
        (6, 10, 6),
