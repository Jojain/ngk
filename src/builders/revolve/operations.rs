//! Revolution operations and edit-scoped implementations.
...
use crate::topology::solid::Solid as SolidView;
...
impl EdgeRevolution {
    ...
    pub fn view<'m, P: Payload>(
        ...
        Ok(EdgeRevolutionView {
            ...
            seam: self.seam.map(|key| model.edge_unchecked(key)),
        })
    }

    /// Checks that this result still belongs to `model`'s committed revision.
    pub fn check<P: Payload>(&self, model: &Model<P>) -> Result<(), StaleResult> {
        StaleResult::check(self.revision, model)
    }
}
...
fn endpoint_origin(source: &RevolvedSourceEdge, endpoint: &RevolvedSourceVertex) -> EditKey {
    endpoint
        .key
        .map(EditKey::Vertex)
        .unwrap_or(EditKey::Edge(source.key))
}
...
pub(crate) fn add_revolved_edge_edit<P: Payload>(
    ...
    let face = if is_full_turn(angle) {
        add_full_revolved_edge_face(edit, &source, axis, angle)?
    } else {
...
fn add_partial_revolved_edge_face<P: Payload>(
    ...
    edit.add_vertex_derived_from(
        vec![endpoint_origin(source, &source.end)],
        VertexAttr::new(end_arc_end, rotated_end),
    );
    edit.add_vertex_derived_from(
        vec![endpoint_origin(source, &source.start)],
        VertexAttr::new(rotated_end_dart, rotated_start),
    );

    edit.add_edge_derived_from(
        vec![endpoint_origin(source, &source.end)],
        EdgeAttr::new(end_arc_start, revolve_circle_curve(axis, end, angle)),
    );
    edit.add_edge_derived_from(
        vec![EditKey::Edge(source.key)],
        EdgeAttr::new(rotated_end_dart, rotated_curve),
    );
    edit.add_edge_derived_from(
        vec![endpoint_origin(source, &source.start)],
        EdgeAttr::new(start_arc_end, revolve_circle_curve(axis, start, angle)),
    );
    ...
    Ok(edit.add_face_derived_from(
        vec![EditKey::Edge(source.key)],
        FaceAttr::with_pcurves(surface, bottom_start, Vec::new(), pcurves),
    ))
}
...
fn add_full_revolved_closed_edge_face<P: Payload>(
    ...
    let face = edit.add_face_derived_from(
        vec![EditKey::Edge(source.key)],
        FaceAttr::closed(surface, cell.anchor(), HashMap::new()),
    );
    cell.own(edit, face);
    Ok(face)
}
...
fn add_full_revolved_apex_to_apex_face<P: Payload>(
    ...
    edit.add_edge_derived_from(
        vec![EditKey::Edge(source.key)],
        EdgeAttr::new(opposite_start, source.curve.clone()),
    );
    if Profile::from_dart(edit, seam_start).is_none() {
        edit.add_profile_derived_from(
            vec![EditKey::Edge(source.key)],
            ProfileAttr::new(seam_start),
        );
    }
    ...
    let face = edit.add_face_derived_from(
        vec![EditKey::Edge(source.key)],
        FaceAttr::with_pcurves(surface, seam_start, Vec::new(), pcurves),
    );
...
fn add_full_revolved_open_edge_face<P: Payload>(
        ...
        .then(|| {
            if reuse_start {
                ...
                add_closed_revolve_boundary_loop(
                    edit,
                    endpoint_origin(source, &source.start),
                    source.start.point,
    ...
    let end_loop = (end_radius > LINEAR_TOLERANCE)
        .then(|| {
            if !reuse_start {
                ...
                add_closed_revolve_boundary_loop(
                    edit,
                    endpoint_origin(source, &source.end),
                    source.end.point,
    ...
    let key = edit.add_face_derived_from(
        vec![EditKey::Edge(source.key)],
        FaceAttr::with_loops(surface, loops, pcurves),
    );
...
fn consume_source_edge_as_closed_loop<P: Payload>(
    ...
    edit.add_vertex_derived_from(
        vec![EditKey::Edge(source.key)],
        VertexAttr::new(start, point),
    );
    edit.add_edge_derived_from(vec![EditKey::Edge(source.key)], EdgeAttr::new(start, curve));
    edit.add_profile_derived_from(vec![EditKey::Edge(source.key)], ProfileAttr::new(start));
    Ok(start)
}
...
fn add_closed_revolve_boundary_loop<P: Payload>(
    ...
    edit.add_vertex_derived_from(vec![source], VertexAttr::new(first, point));
    edit.add_edge_derived_from(vec![source], EdgeAttr::new(first, curve));
    edit.add_profile_derived_from(vec![source], ProfileAttr::new(first));
    Ok(first)
}
...
pub(crate) fn add_revolved_profile_edit<P: Payload>(
    ...
    let sheet = edit.add_sheet_derived_from(
        vec![EditKey::Profile(profile)],
        SheetAttr::new(built.swept_dart),
    );
...
fn add_revolved_profile_from_dart_edit<P: Payload>(
    ...
    axis: Axis3,
    angle: Rad64,
) -> Result<RevolvedProfileBuild, RevolveError> {
    ...
    add_revolved_profile_faces(edit, profile_dart, axis, angle, close_ring)
}

struct RevolvedProfileBuild {
    swept_dart: Dart,
...
fn add_revolved_edge_face<P: Payload>(
    ...
    axis: Axis3,
    angle: Rad64,
) -> Result<RevolvedFaceBuild, RevolveError> {
    validate_revolvable_radii(axis, source)?;
...
fn add_revolved_quad_face<P: Payload>(
    ...
    for i in 0..4 {
        ...
        edit.add_vertex_derived_from(vec![corner_origins[i]], VertexAttr::new(dart, corners[i]));
    }

    for i in 0..4 {
        let edge_dart = darts[2 * i];
        edit.add_edge_derived_from(
            vec![corner_origins[i]],
            EdgeAttr::new(edge_dart, boundary_curves[i].clone()),
        );
    }

    edit.add_profile_derived_from(vec![EditKey::Edge(source)], ProfileAttr::new(darts[0]));
    let key = edit.add_face_derived_from(
        vec![EditKey::Edge(source)],
...
fn add_full_revolved_band_face<P: Payload>(
    ...
    for (seed, point, curve, origin) in [
        (
            start_first,
            ends[0],
            &circles[0],
        ...
        edit.add_vertex_derived_from(vec![origin], VertexAttr::new(seed, point));
        edit.add_edge_derived_from(vec![origin], EdgeAttr::new(seed, curve.clone()));
        edit.add_profile_derived_from(vec![origin], ProfileAttr::new(seed));
    }
...
pub(crate) fn add_revolved_face_edit<P: Payload>(
    ...
    if edit.sheet_key(shell).is_none() {
        edit.add_sheet_derived_from(vec![EditKey::Face(face_key)], SheetAttr::new(shell));
    }
    Ok(FaceRevolution {
        solid: edit
            .add_solid_derived_from(vec![EditKey::Face(face_key)], SolidAttr::new(shell, None)),
        start_cap: Some(face_key),
        end_cap: Some(top_face_key),
        laterals: lateral_faces,
        seam: None,
        revision: None,
    })
}
...
fn add_full_revolved_face<P: Payload>(
    ...
    Ok(edit.add_solid_derived_from(vec![EditKey::Face(face_key)], SolidAttr::new(shell, None)))
}
...
mod tests {
    ...
    use super::add_revolved_edge_edit;
    ...
    #[test]
    fn partial_edge_revolve_declares_endpoint_and_source_lineage() {
        ...
        let start_dart = model.edge_attr_unchecked(edge).dart;
        let start = model
            .cell_key::<Cell0>(start_dart)
            .expect("source start should be a vertex");
        let end = model
            .cell_key::<Cell0>(model.alpha(crate::topology::gmap::Dim::Zero, start_dart))
            .expect("source end should be a vertex");
        let mut recorder = LineageRecorder::default();

        model
            .transaction_with_policy(&mut recorder, |edit| {
                add_revolved_edge_edit(
                    edit,
                    edge,
            ...
            .expect("partial revolve should commit");

        assert!(
            recorder
                .created
                .iter()
                .any(|(_, origin)| *origin == Origin::derived(EditKey::Edge(edge)))
        );
        assert_eq!(
            recorder
                .created
                .iter()
                .filter(|(_, origin)| *origin == Origin::derived(EditKey::Vertex(start)))
                .count(),
            2
        );
        assert_eq!(
            recorder
                .created
                .iter()
                .filter(|(_, origin)| *origin == Origin::derived(EditKey::Vertex(end)))
                .count(),
            2
        );
        assert_eq!(
            recorder
                .created
                .iter()
                .filter(|(_, origin)| *origin == Origin::derived(EditKey::Edge(edge)))
                .count(),
            3
        );
    }
