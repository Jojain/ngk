...
pub(in crate::healing::passes) fn plan<P: Payload>(
    ...
    let (survivor, consumed) = match incident_faces(g, dart)[..] {
        ...
        [face] => (face, None),
        // Matches the survivor rule of `remove_cell_edit`.
        [first, second]
    ...
    let surfaces = match consumed {
        ...
        // wrapping loops the face really has. `remove_cell_edit` recognises
...
pub(in crate::healing::passes) fn apply<P: Payload>(
    ...
    let removal = remove_cell_edit(edit, fusion.dart, Dim::One)?;
