...
use super::edge::Edge;
use super::face::Face;
use super::gmap::{Cell2, Dart, GMap, MergeHandle, MergeTopology, TopologyMerge};
...
impl<'a, P: Payload> Sheet<'a, P> {
    ...
    pub fn reversed(&self) -> Self {
        let anchor = match self.anchor {
            SheetAnchor::Dart(dart) => SheetAnchor::Dart(self.gmap.alpha(Dim::Zero, dart)),
    ...
    pub fn darts(&self) -> impl Iterator<Item = Dart> + '_ {
        self.dart()
            .map(|dart| self.gmap.sheet_darts(dart))
            .unwrap_or_default()
            .into_iter()
    }
    ...
    pub fn faces(&self) -> Vec<Face<'a, P>> {
        let seed = match self.anchor {
            ...
            SheetAnchor::Face(face, sense) => {
                let face = Face::new(self.gmap, face);
...
impl<P: Payload> MergeTopology<P> for Sheet<'_, P> {
    fn merge_topology(&self) -> TopologyMerge<'_, P> {
        match self.anchor {
            SheetAnchor::Dart(dart) => TopologyMerge::new(self.gmap, self.darts().collect(), dart),
